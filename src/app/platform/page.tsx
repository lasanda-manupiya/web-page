  ...
  return (
    <>
      ...
      <header className="mx-auto max-w-content px-4 pt-4">
        <h1 className="text-3xl font-bold text-navy-900 md:text-4xl">Integrated BIM project intelligence platform</h1>
      ...
      <section className="mx-auto max-w-content px-4 py-8" aria-labelledby="layers-h">
        <h2 id="layers-h" className="text-2xl font-bold text-navy-900">Four connected layers</h2>
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          {layers.map(([t, b]) => (
            <div key={t} className="rounded-panel border border-navy-200 bg-white p-4">
              <h3 className="font-semibold text-navy-900">{t}</h3>
              <p className="mt-1 text-sm text-neutral-700">{b}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-content px-4 py-4" aria-labelledby="cap-h">
        <h2 id="cap-h" className="text-2xl font-bold text-navy-900">Capabilities</h2>
        <ul className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {CAPABILITIES.map((c) => (
            <li key={c.url} className="rounded-panel border border-navy-200 p-4 hover:bg-navy-50">
              <Link href={c.url} className="font-medium text-navy-900 hover:underline">{c.h1}</Link>
              <p className="mt-1 text-sm text-neutral-700">{c.intent}</p>
            </li>
          ))}
        </ul>
      </section>
      ...
      <section className="mx-auto max-w-content px-4 py-8" aria-labelledby="sol-h">
        <h2 id="sol-h" className="text-2xl font-bold text-navy-900">Solutions by organisation</h2>
        <ul className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {SOLUTIONS.map((s) => (
            <li key={s.url} className="rounded-panel border border-navy-200 p-4 hover:bg-navy-50">
              <Link href={s.url} className="font-medium text-navy-900 hover:underline">{s.h1}</Link>
              <p className="mt-1 text-sm text-neutral-700">{s.intent}</p>
            </li>
          ))}
        </ul>
      </section>
      ...
      <div className="mx-auto max-w-content px-4 pb-12">
        <Link href="/3d-demo" className="rounded-panel bg-navy px-5 py-3 font-medium text-white hover:bg-navy-700">
